...
public class TimerTaskScheduling {
    ...
    public void startThread(String threadName, long time) {
        ...
        synchronized(lock) {
            queue.offer(new AbstractMap.SimpleEntry<>(time, new Thread(threadName) {
                public void run() {
    ...
    public static class DispatchThread extends Thread {
        private Object lock;
        private PriorityQueue<Entry<Long, Thread>> queue;
        ...
        public void run() {
            ...
            //for the next thread in the min-heap.
            //(Because of deletions, it may happen that the dispatch thread wakes up and finds nothing to do.)
            try {
                while (true) {
                    synchronized(lock) {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        timerTaskScheduling.startThread("Thread2", epochTime+20);
